...
export default function LandingPage() {
  ...
  <div style={{ maxWidth: 1200, margin: "0 auto", padding: "14px 24px", display: "flex", alignItems: "center", gap: 16 }}>
    ...
    <div className="nx-scroll-nav" style={{ display: "flex", alignItems: "center", gap: 22, overflowX: "auto", WebkitOverflowScrolling: "touch", flex: 1 }}>
      {NAV.map((n) => (
        <a key={n} href={`#${n.toLowerCase().replace(/\s/g, "-")}`} style={{ color: C.ink, fontSize: 13.5, fontWeight: 600, textDecoration: "none", letterSpacing: ".02em", whiteSpace: "nowrap", flexShrink: 0 }}>
          {n.toUpperCase()}
        </a>
      ))}
      <a href="/login" style={{ border: `1.5px solid ${C.navy}`, color: C.navy, borderRadius: 10, padding: "9px 18px", fontWeight: 700, fontSize: 13.5, textDecoration: "none", whiteSpace: "nowrap", flexShrink: 0 }}>LOGIN</a>
      <a href="/login" style={{ background: C.gold, color: "#1C2430", borderRadius: 10, padding: "9px 18px", fontWeight: 700, fontSize: 13.5, textDecoration: "none", whiteSpace: "nowrap", flexShrink: 0 }}>REGISTER</a>
    </div>
  </div>
</header>
